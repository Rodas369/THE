const JAMENDO_CLIENT_ID = '56d30c95' // Client ID público para más contenido
...
class MusicApiService {
  ...
  async searchTracks(query: string, limit: number = 20): Promise<Track[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=${limit}&search=${encodeURIComponent(query)}&include=musicinfo&audiodownload=mp32&groupby=artist_id`
      )
  ...
  async getPopularTracks(limit: number = 20): Promise<Track[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=${limit}&order=popularity_total&include=musicinfo&audiodownload=mp32`
      )
      const data: JamendoResponse = await response.json()
      return data.results || []
    } catch (error) {
  ...
  async getTracksByGenre(genre: string, limit: number = 20): Promise<Track[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=${limit}&tags=${genre}&include=musicinfo&audiodownload=mp32`
      )
      const data: JamendoResponse = await response.json()
      return data.results || []
    } catch (error) {
  ...
  async getNewReleases(limit: number = 20): Promise<Track[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=${limit}&order=releasedate_desc&include=musicinfo&audiodownload=mp32`
      )
      const data: JamendoResponse = await response.json()
      return data.results || []
    } catch (error) {
  ...
  convertToSong(track: Track) {
    return {
      ...
      audio_url: track.audiodownload || track.audio || 'https://www.soundjay.com/misc/sounds/bell-ringing-05.wav',
